...
def _create_spectral_callable(spectral_dict: Dict[str, str]) -> Callable[[KernelContext], float]:
    ...
    if not Path(file_path).is_absolute():
        ...
        generator_dir = Path(s2gos_generator.__file__).parent
    ...
    try:
        ...
        if variable not in dataset.data_vars:
            raise KeyError(f"Variable '{variable}' not found in dataset. Available: {list(dataset.data_vars.keys())}")
        ...
        logging.info(f"Successfully loaded spectral data: {full_path} (variable: {variable})")
        return spectral_func
        
    except Exception as e:
        # Log the error and fallback 
        logging.error(f"Failed to load spectral data from {full_path} (variable: {variable}): {e}")
        logging.warning(f"Using fallback constant value 0.5 for spectral parameter")
...
class EradiateMaterialAdapter:
    ...
    @staticmethod
    def load_spectral_data(file_path: Path, variable: str = "reflectance"):
    ...
    def create_diffuse_kdict(material) -> dict:
        ...
        result = {
            material.mat_id: {"type": "diffuse", "id": material.mat_id, "reflectance": 0.0}
        }
